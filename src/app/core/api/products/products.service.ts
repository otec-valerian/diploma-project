...
export class ProductsService {
    ...
    public getProducts(): Observable<Product[]> {
        ...
        return of(JSON.parse(localStorage.getItem('products') as string)).pipe(delay(1000));
    }
    ...
    public getRecommendedProducts(): Observable<Product[]> {
        ...
        svd.train(feedbackMatrix);

        let predictions: any = []
        feedbackMatrix[userIndex].forEach((productFeedback: any, i:number) => {
            if (productFeedback === 0) {
                console.log('test', userIndex, i, svd.predict(userIndex, i))
        ...
        return of([recommendedProduct]).pipe(delay(1000))
    }

    public buyProduct(productUuid: string): Observable<Product> {
        // return this.httpClient.post<Product>(`${this.apiUrl}`, {uuid: productUuid});
        return of(JSON.parse(localStorage.getItem('products') as string).find((product: any) => product.uuid === productUuid)).pipe(delay(1000));
    }
    ...
    public setProductFeedback(request: {uuid: string, mark: number}): Observable<Product> {
        ...
        return of(products[productIndex]).pipe(delay(1000));
    }
}
